...
export const Skills: React.FC = () => {
  ...
  return <section id="skills" className="py-20 bg-[#08090D] relative border-t border-[#1F2430]/60">
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.3 }} transition={{ duration: 0.5 }} className="flex flex-col items-center text-center mb-12">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.3 }} transition={{ duration: 0.5, delay: 0.1 }} className="mb-10 p-6 rounded-2xl bg-[#101218] border border-[#1F2430] glass-panel">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">{visible.map((group, index) => <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.1 }} transition={{ duration: 0.4, delay: 0.1 + (index * 0.1) }} key={group.category} className="p-6 rounded-2xl bg-[#101218] border border-[#1F2430] glass-panel-hover hover-lift"><div className="flex items-center gap-3 pb-4 mb-4 border-b border-[#1F2430]"><div className="p-2.5 rounded-xl bg-[#151821] border border-[#1F2430]">{icons[group.category]}</div><h3 className="text-base font-bold text-white">{group.category}</h3></div><div className="flex flex-wrap gap-2">{group.skills.map((skill) => <span key={skill} className="px-3 py-1.5 rounded-lg bg-[#151821] border border-[#1F2430] text-xs text-slate-300 font-mono">{skill}</span>)}</div></motion.div>)}</div>
    </div>
  </section>;
};
